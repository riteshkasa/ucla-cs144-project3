...
const seed = db.transaction(() => {
  ...
  insertLogin.run('123456789', 'Alice Johnson', hashPw('alice123'), 'student');
  insertLogin.run('987654321', 'Bob Smith', hashPw('bob456'), 'student');
  insertLogin.run('555123456', 'Charlie Davis', hashPw('charlie789'), 'student');
  insertLogin.run('profrosario', 'Rosario', hashPw('bruin'), 'professor');
...
db.exec(`
  CREATE TABLE student_courses AS
    SELECT e.login_uid, c.id AS course_id, c.code AS course_code, c.title AS course_title, c.instructor
    FROM enrollment e
    JOIN course c ON c.id = e.course_id;

  CREATE TABLE professor_courses AS
    SELECT c.professor_uid, c.id AS course_id, c.code AS course_code, c.title AS course_title, c.instructor
    FROM course c
    WHERE c.professor_uid IS NOT NULL;

  CREATE TABLE course_content AS
    SELECT w.course_id, w.id AS week_id, w.title AS week_title, w.sort_order AS week_sort,
           e.id AS entry_id, e.title AS entry_title, e.type AS entry_type, e.url AS entry_url, e.sort_order AS entry_sort
    FROM week w
    JOIN entry e ON e.week_id = w.id;

  CREATE TABLE course_students AS
    SELECT e.course_id, l.uid, l.name
    FROM enrollment e
    JOIN login l ON l.uid = e.login_uid;

  CREATE TABLE student_grades AS
    SELECT g.login_uid, a.course_id, g.id AS grade_id, a.id AS assignment_id, a.name AS assignment_name, g.score, a.sort_order
    FROM grade g
  ...
  CREATE INDEX idx_student_grades_login_uid_course_id ON student_grades(login_uid, course_id);
`);
